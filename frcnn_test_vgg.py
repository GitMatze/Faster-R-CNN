# -*- coding: utf-8 -*-
"""FRCNN_TEST_all_ifn_data.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1MwXoZFiaoAr2PPPNbrO4IP1EWhw5mMi_
"""
...
print('Annotation File for Testin: {}'.format(anno_path_test))
print('Image Data: {}'.format(data_path))
print('Output: {}'.format(test_store_path))
print('Number of Test Images Saved For Visualization: {}'.format(args.num_test_imgs))
...
def rpn_to_roi(rpn_layer, regr_layer, C, dim_ordering, use_regr=True, max_boxes=300,overlap_thresh=0.9):
	...
	(rows, cols) = rpn_layer.shape[1:3]  # zweite und dritte Stelle
	...
	for anchor_size in anchor_sizes:
		for anchor_ratio in anchor_ratios:
			...
			A[2, :, :, curr_layer] = np.minimum(cols, A[2, :, :, curr_layer]) #TODO: das war vorher cols-1, rows-1, aber so bessere Bildabdeckung
			A[3, :, :, curr_layer] = np.minimum(rows, A[3, :, :, curr_layer])
...
assert ( os.path.exists(config_output_filename) ),"Config File {} missing, Check if training has been performed with given session name".format(config_output_filename)
...
fig.suptitle('Training Results: {}'.format(args.session_name), fontsize=55)
...
plt.subplot(4,2,2)
plt.plot(np.arange(0, r_epochs), record_df['class_acc'], label = 'Training Data' )
plt.plot(np.arange(0, r_epochs), record_df['class_acc_val'], label = 'Validation Data')
plt.title('class_acc')
ax = plt.gca()
ax.legend()

plt.subplot(4,2,3)
plt.plot(np.arange(0, r_epochs), record_df['loss_rpn_cls'] )
plt.plot(np.arange(0, r_epochs), record_df['loss_rpn_cls_val'] )

plt.title('loss_rpn_cls')

plt.subplot(4,2,4)
plt.plot(np.arange(0, r_epochs), record_df['loss_rpn_regr'])
plt.plot(np.arange(0, r_epochs), record_df['loss_rpn_regr_val'])

plt.title('loss_rpn_regr')


plt.subplot(4,2,5)
plt.plot(np.arange(0, r_epochs), record_df['loss_class_cls'])
plt.plot(np.arange(0, r_epochs), record_df['loss_class_cls_val'])

plt.title('loss_class_cls')

plt.subplot(4,2,6)
plt.plot(np.arange(0, r_epochs), record_df['loss_class_regr'])
plt.plot(np.arange(0, r_epochs), record_df['loss_class_regr_val'])

plt.title('loss_class_regr')

plt.subplot(4,2,7)
plt.plot(np.arange(0, r_epochs), record_df['curr_loss'])
plt.plot(np.arange(0, r_epochs), record_df['curr_loss_val'])

plt.title('total_loss')

plt.subplot(4,2,8)
plt.plot(np.arange(0, r_epochs), record_df['elapsed_time'])
plt.title('elapsed_time')

#plt.show()
fig.savefig(os.path.join(test_store_path, 'results.pdf'))
"""# Test"""
...
#     print(img_path)
#     st = time.time()
#     filepath = os.path.join(test_base_path, img_path)
#
...
def plot_region_proposals(R,ax,num,C):
	'''
	R: boxes, ax: axis, num: Anzahl zu plottender Boxen, C: config Datei
	'''
	for i in random.sample(range(len(R)), num):
		box = R[i]
		f = C.rpn_stride
		x1, y1, w, h = get_real_coordinates(ratio, f*box[0], f*box[1], f*box[2], f*box[3])
		p = patches.Rectangle((x1,y1),w,h, linewidth=4,
