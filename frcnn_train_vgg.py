...
Original file is located at
	https://colab.research.google.com/drive/1Cux-t1yOJn5cZ6cO36P-EQ1_njVJGRwP
...
class Config:
	...
	# Original anchor_box_scales in the paper is [128, 256, 512]
		...
		# overlaps for RPN                                                          #TODO What does that mean?
...
"""#### Definitions of Methods for IoU Calculation"""
...
"""#### calc_rpn Definition 
*Calculates the rpns for all anchors, e.g. Y (classifier and boxregression based on ground truth)
"""
...
"""#### get_new_img_size() and augment() Definition
...
"""#### Definition of non max supression"""
...
"""#### apply_regr / apply_regr_np() Definitionen"""
...
if not os.path.isfile(C.model_path):
	...
	record_df = pd.DataFrame(columns=['mean_overlapping_bboxes',
				 'class_acc', 'loss_rpn_cls', 'loss_rpn_regr', 'loss_class_cls', 'loss_class_regr', 'curr_loss',
				 'class_acc_val', 'loss_rpn_cls_val', 'loss_rpn_regr_val', 'loss_class_cls_val', 'loss_class_regr_val', 'curr_loss_val',
				 'elapsed_time', 'mAP'])
else:
...
if len(record_df)==0:
	...
	best_loss = np.min(r_curr_loss) #ToDO: r_curr_loss_val for early stopping
...
def validate(val_length):
	losses_val = np.zeros((val_length, 5))
	...
	for iter_num in range(val_length):
		...
		iter_num += 1 #for last expession we need to count from one on
		progbar.update(iter_num,
					   [('rpn_cls_val', np.mean(losses_val[:iter_num, 0])), ('rpn_regr_val', np.mean(losses_val[:iter_num, 1])),
						('final_cls_val', np.mean(losses_val[:iter_num, 2])), ('final_regr_val', np.mean(losses_val[:iter_num, 3]))])

	val = {}
	val['loss_rpn_cls'] = np.mean(losses_val[:, 0])
	val['loss_rpn_regr'] = np.mean(losses_val[:, 1])
	val['loss_class_cls'] = np.mean(losses_val[:, 2])
	val['loss_class_regr'] = np.mean(losses_val[:, 3])
	val['class_acc'] = np.mean(losses_val[:, 4])
	val['curr_loss'] = sum([val[key] for key in val])
		...
		try:
		   ...
		   # R: bboxes (shape=(300,4))
			...
			if iter_num == epoch_length:
				...
				val_success = False
				while val_success == False:
					try:
						val_losses = validate(args.val_length)
						val_success = True
					except Exception as e:
						print(e)
				...
				# if val_losses['curr_loss'] < best_loss:  #note that initialization was also changed
				if curr_loss < best_loss:
					if C.verbose:
						print('Total total loss decreased from {} to {}, saving weights'.format(best_loss,curr_loss))
					best_loss = curr_loss
				...
				new_row = {'mean_overlapping_bboxes':round(mean_overlapping_bboxes, 3),
						   ...
						   'class_acc_val': round(val_losses['class_acc'], 3),
						   'loss_rpn_cls_val': round(val_losses['loss_rpn_cls'], 3),
						   'loss_rpn_regr_val': round(val_losses['loss_rpn_regr'], 3),
						   'loss_class_cls_val': round(val_losses['loss_class_cls'], 3),
						   'loss_class_regr_val': round(val_losses['loss_class_regr'], 3),
						   'curr_loss_val': round(val_losses['curr_loss'], 3),
			...
			print('Exception: {};'.format(e), 'Error on line {}'.format(sys.exc_info()[-1].tb_lineno))
			continue
